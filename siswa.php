        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            if (mysqli_num_rows($cek) == 0 && !empty($nisn)) {
                ...
                if (mysqli_query($conn, $sql_siswa)) {
                    ...
                    $sql_user = "INSERT INTO users (username, password, role, nama_lengkap) VALUES ('$nisn', '$pass_default', 'siswa', '$nama')";
...
if (isset($_GET['hapus'])) {
    ...
    if(mysqli_num_rows($q_cek) > 0) {
        ...
        echo "<script>alert('Data dihapus.'); window.location='siswa.php';</script>";
    }
}
if (isset($_GET['reset'])) {
    $id_siswa = $_GET['reset'];
    ...
    if (mysqli_num_rows($q_cari) > 0) {
        ...
        if ($reset) echo "<script>alert('Password direset ke NISN.'); window.location='siswa.php';</script>";
    }
}
...
if (isset($_POST['simpan']) || isset($_POST['update'])) {
    ...
    if (isset($_POST['simpan'])) {
        ...
        if (mysqli_num_rows($cek_nisn) > 0) { $pesan = "<div class='alert alert-danger'>NISN terdaftar.</div>"; } 
        else {
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                $pesan = "<div class='alert alert-success'>Data Siswa ditambahkan.</div>";
            }
        }
        ...
        if (mysqli_query($conn, $sql_update)) {
            ...
            echo "<script>alert('Data Diupdate!'); window.location='siswa.php';</script>";
        }
    }
}
...
<div class="d-flex" id="wrapper">
    ...
    <div id="page-content-wrapper">
        <nav class="navbar navbar-expand-lg navbar-light bg-white border-bottom shadow-sm px-4">
            ...
            <div class="collapse navbar-collapse"><ul class="navbar-nav ms-auto mt-2 mt-lg-0"><li class="nav-item"><span class="nav-link fw-bold text-secondary">Halo, <?= $_SESSION['nama'] ?> (<?= strtoupper($role) ?>)</span></li></ul></div>
        </nav>
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="card shadow-sm border-success mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" enctype="multipart/form-data">
                                <div class="mb-3">
                                    <input type="file" name="file_siswa" class="form-control" required accept=".csv">
                                </div>
                    ...
                    <div class="card shadow-sm border-0 mb-4">
                        <div class="card-header <?= $edit_mode ? 'bg-warning text-dark' : 'bg-primary text-white' ?>">
                            <h5 class="mb-0"><i class="fa-solid <?= $edit_mode ? 'fa-pen-to-square' : 'fa-user-plus' ?> me-2"></i><?= $edit_mode ? 'Edit Siswa' : 'Input Manual' ?></h5>
                        </div>
                        <div class="card-body">
                            <?= $pesan ?>
                            <form method="POST" action="siswa.php">
                                ...
                                <?php endif; ?>
                                <div class="mb-2"><label class="form-label">NISN</label><input type="number" name="nisn" class="form-control" required placeholder="12345" value="<?= $edit_mode ? $data_edit['nisn'] : '' ?>"></div>
                                <div class="mb-2"><label class="form-label">Nama Lengkap</label><input type="text" name="nama" class="form-control" required value="<?= $edit_mode ? $data_edit['nama'] : '' ?>"></div>
                                <div class="mb-2"><label class="form-label">Jenis Kelamin</label>
                                    <select name="jk" class="form-select" required>
                                        <option value="">Pilih...</option><option value="L" <?= ($edit_mode && $data_edit['jenis_kelamin'] == 'L') ? 'selected' : '' ?>>Laki-laki</option><option value="P" <?= ($edit_mode && $data_edit['jenis_kelamin'] == 'P') ? 'selected' : '' ?>>Perempuan</option>
                                    </select>
                                </div>
                                <div class="mb-2"><label class="form-label">Kelas</label>
                                    <select name="kelas" class="form-select" required>
                                        <option value="">Pilih</option><?php $kel = ['X','XI','XII']; foreach($kel as $k) { $sel = ($edit_mode && $data_edit['kelas'] == $k) ? 'selected' : ''; echo "<option value='$k' $sel>$k</option>"; } ?>
                                    </select>
                                </div>
                                <div class="mb-2"><label class="form-label">Jurusan</label>
                                    <select name="jurusan" class="form-select" required>
                                        <option value="">Pilih</option><?php $jur = ['RPL','TKJ','MM']; foreach($jur as $j) { $sel = ($edit_mode && $data_edit['jurusan'] == $j) ? 'selected' : ''; echo "<option value='$j' $sel>$j</option>"; } ?>
                                    </select>
                                </div>
                                <div class="mb-3"><label class="form-label">No HP Ortu</label><input type="text" name="hp" class="form-control" required value="<?= $edit_mode ? $data_edit['no_hp_ortu'] : '' ?>"></div>
                                <div class="d-grid gap-2">
                                    <button type="submit" name="<?= $edit_mode ? 'update' : 'simpan' ?>" class="btn <?= $edit_mode ? 'btn-warning' : 'btn-primary' ?>"><?= $edit_mode ? 'Simpan Perubahan' : 'Simpan Data' ?></button>
                                    <?php if($edit_mode): ?><a href="siswa.php" class="btn btn-secondary">Batal Edit</a><?php endif; ?>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                <div class="col-md-8">
                    <div class="card shadow-sm border-0">
                        <div class="card-header bg-white"><h5 class="mb-0"><i class="fa-solid fa-list me-2"></i>Data Siswa Terdaftar</h5></div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-hover align-middle">
                                    <thead class="table-dark"><tr><th>NISN</th><th>Nama</th><th>L/P</th><th>Kelas</th><th class="text-center">Aksi</th></tr></thead>
                                    <tbody>
                                        <?php
                                        ...
                                        <tr class="<?= ($edit_mode && $data_edit['id'] == $r['id']) ? 'table-warning' : '' ?>">
                                            <td><?= $r['nisn'] ?></td><td><?= $r['nama'] ?></td><td><?= $r['jenis_kelamin'] ?></td><td><?= $r['kelas'] ?> - <?= $r['jurusan'] ?></td>
                                            <td class="text-center">
                                                <div class="btn-group btn-group-sm">
                                                    ...
                                                    <a href="siswa.php?hapus=<?= $r['id'] ?>" class="btn btn-outline-danger" onclick="return confirm('Yakin hapus?')" title="Hapus"><i class="fa-solid fa-trash"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                        <?php } ?>
